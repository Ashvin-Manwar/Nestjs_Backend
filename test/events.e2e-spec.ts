...
import * as request from "supertest"//http testing library
...
const sql=fs.readFileSync(path.join(__dirname,'fixtures',sqlFileName),
 ...
 for(const c of sql.split(';')){
    await queryRunner.query(c)//to run query file
 }
}
...
describe('Events (e2e)',()=>{
    beforeEach(async ()=>{//newly added
    // beforeAll(async ()=>{
        //app initialisation in module
        mod =await Test.createTestingModule({
            imports:[AppModule]
        }).compile()
        app = mod.createNestApplication()
        app.useGlobalPipes(new ValidationPipe)//new ly added
        await app.init()

        connection=app.get(Connection)//get Dependancy injection to get current connection object

    })
    //stop the app
    afterEach(async ()=>{//Modified
    // afterAll(async ()=>{
        await app.close()
    })
